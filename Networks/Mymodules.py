...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SBlock(dim=dim, input_resolution=input_resolution,
...
class FocalModulation(nn.Module):
    ...
    def __init__(self, dim, proj_drop=0., focal_level=2, focal_window=7, focal_factor=2, use_postln=False):
        ...
        self.conv1 = Conv1(dim, 2 * dim + (self.focal_level + 1))
        self.act = nn.GELU()
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)
    ...
    def forward(self, x):
        ...
        q, ctx, gates = torch.split(x1, (C, C, self.focal_level + 1), 1)
        ...
        x2 = x_out.permute(0, 3, 1, 2).contiguous()
        return x2
...
class MODEL(nn.Module):
    def __init__(self, img_size=120,patch_size=4,embed_dim=96,num_heads=8, window_size=1,in_channel=2, out_channel=16,output_channel=1,
        ...
        self.focal=FocalModulation(out_channel)
    ...
    def forward(self, input):
        ...
        focal= self.focal(convolutioanl )

        encode_size = ( focal.shape[2],  focal.shape[3])
        swinTransformer=self.basicLayer( focal, encode_size)



        out=self.convolutional_out (swinTransformer)
        return out


class DIS(nn.Module):
    def __init__(self, img_size=120,patch_size=4,embed_dim=96,num_heads=8, window_size=1,in_channel=1, out_channel=16,output_channel=1,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., patch_norm=True,depth=2,downsample=None,
                 drop_path=0., norm_layer=nn.LayerNorm,use_checkpoint=False ):
    ...
    def forward(self, input):
        ...
        encode_size = (convolutioanl.shape[2], convolutioanl.shape[3])
        swinTransformer = self.basicLayer(convolutioanl, encode_size)
